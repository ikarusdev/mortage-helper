...
export default class HMortgage {
    ...
    getNextValues(sum){
        const mInterestRate = this.getMonthlyInterestRate();
        const mPayments = this.getMonthlyMortgagePayment();

        const interest = HMortgage.round((this.principal - sum) * mInterestRate);
        const principal = mPayments - interest;
    ...
    getMonthlyMortgagePayment() {
        const mInterestRate = this.getMonthlyInterestRate();
        const powed = Math.pow(1 + mInterestRate, this.term);
        return HMortgage.round(this.principal * ((mInterestRate * powed) / (powed - 1)));
    }
}